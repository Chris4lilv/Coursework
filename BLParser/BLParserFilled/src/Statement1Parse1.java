...
public final class Statement1Parse1 extends Statement1 {
    ...
    private static void parseIf(Queue<String> tokens, Statement s) {
        ...
        Reporter.assertElseFatalError(Tokenizer.isCondition(tokens.front()),
                "Condition must follow IF");
        Condition c = parseCondition(tokens.dequeue());
        Reporter.assertElseFatalError(tokens.dequeue().equals("THEN"),
                "Missing THEN keyword");
        Statement sTemp = s.newInstance();
        sTemp.parseBlock(tokens);
        //check if this is an if else
        if (tokens.front().equals("ELSE")) {
            tokens.dequeue(); //no need to check, must be an else if block run
            Statement sTempElse = s.newInstance();
            sTempElse.parseBlock(tokens);
            s.assembleIfElse(c, sTemp, sTempElse);
        } else {
            s.assembleIf(c, sTemp);
        }
    ...
    public void parseBlock(Queue<String> tokens) {
        ...
        while (Tokenizer.isIdentifier(tokens.front())
            ...
            temp.parse(tokens);
            this.addToBlock(this.lengthOfBlock(), temp);
            temp.clear(); //clear temp to make the next block to add to THIS
        }
