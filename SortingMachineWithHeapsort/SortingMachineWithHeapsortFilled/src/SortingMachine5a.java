...
public class SortingMachine5a<T> extends SortingMachineSecondary<T> {
    ...
    private static <T> void siftDown(Array<T> array, int top, int last,
        ...
        int right = top * 2 + 2;
        if (right <= last) {
            //if the right is less than or equal to last, both left
            //and right are elements
            if (order.compare(array.entry(left), array.entry(right)) <= 0) {
                //left is the smaller element of left and right
                if (order.compare(array.entry(left), array.entry(top)) < 0) {
                    //left is smaller than the root
                    array.exchangeEntries(left, top);
                    siftDown(array, left, last, order);
                }

            } else {
                //right is the smaller element of left and right
                if (order.compare(array.entry(right), array.entry(top)) < 0) {
                    //right is smaller than the root
                    array.exchangeEntries(right, top);
                    siftDown(array, right, last, order);
                }
            }
        } else if (left <= last) {
            //here only left is an non empty element
            if (order.compare(array.entry(left), array.entry(top)) < 0) {
                //this was the last element, swap them and be done
                array.exchangeEntries(left, top);
            }

        }
        //else do nothing, there is no left and right element
    ...
    private static <T> Array<T> buildHeap(Queue<T> q, Comparator<T> order) {
        ...
        Array<T> heaparr = new Array1L<>(q.length());
        int i = 0;
        while (q.length() > 0) {
            heaparr.setEntry(i, q.dequeue());
            i++;
        }
        heapify(heaparr, 0, order);
        return heaparr;
    }
