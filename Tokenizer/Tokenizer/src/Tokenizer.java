    ...
    private static String nextWordOrSeparator(String text, int position) {
        ...
        if (SEPARATORS.contains(text.charAt(position))) {
            while (i < text.substring(position, text.length()).length()) {
                singleChar = text.charAt(position + i);
                //make sure the next character is contained in the set
                if (SEPARATORS.contains(text.charAt(position + i))) {
                    nextWordOrSeparator = nextWordOrSeparator + singleChar;
                    i++;
                } else {
            ...
            while (i < text.substring(position, text.length()).length()) {
                singleChar = text.charAt(position + i);
                //make sure the next character is not contained in the set
                if (!SEPARATORS.contains(text.charAt(position + i))) {
                    nextWordOrSeparator = nextWordOrSeparator + singleChar;
                    i++;
                } else {
