...
public class Map4<K, V> extends MapSecondary<K, V> {
    ...
    public final void add(K key, V value) {
        ...
        Map<K, V> bucketMap = this.hashTable.entry(bucket);
    ...
    public final Pair<K, V> removeAny() {
        ...
        int i = 0;
        Pair<K, V> ret = null; //needs to be initialized with a value.
        while (searching) {
            Map<K, V> map = this.hashTable.entry(i);
            if (map.size() > 0) {
                ret = map.removeAny();
                searching = false;
            }
