...
public final class JCFExplorations {
    ...
    public static void giveRaise(components.map.Map<String, Integer> map,
        ...
        while (map.size() > 0) {
            ...
            if (pair.key().equals(initial)) {
                newMap.add(pair.key(), pair.value() * (1 + raisePercent));
            } else {
    ...
    public static void giveRaise(java.util.Map<String, Integer> map,
        ...
        for (java.util.Map.Entry<String, Integer> entry : map.entrySet()) {
            if (entry.getKey().equals(initial)) {
                entry.setValue(entry.getValue() * (1 + raisePercent));
                //or map.put(entry.getKey(), entry.getValue() * (1 + raisePercent));??
            }
        }
    }
    ...
    public static void incrementAll(components.set.Set<NaturalNumber> set) {
        ...
        Set<NaturalNumber> newSet = set.newInstance();
        while (set.size() > 0) {
            NaturalNumber nn = set.removeAny();
            nn.increment();
            newSet.add(nn);
        }
    ...
    public static void incrementAll(java.util.Set<NaturalNumber> set) {
        ...
        while (it.hasNext()) {
            NaturalNumber nn = (NaturalNumber) it.next();
            nn.increment();
        }
        //????
